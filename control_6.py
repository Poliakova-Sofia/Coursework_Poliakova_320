...
class VideoApp:
    ...
    def update_frames(self):
        ...
        if ret:
            ...
            self.frame = frame  # save the original frame

            # displaying the original frame on the left
    ...
    def start_videos(self):
        self.update_frames()  # start the video display
    ...
    def __init__(self, root):
        ...
        self.capp = cv2.VideoCapture('Avia.mp4')
        ...
        # video start button
        self.btn_load_left = tk.Button(self.checkbox_frame, text="Load the video", bg="light blue", command=self.start_videos)
        self.btn_load_left.grid(row=0, column=0, sticky="w")

        # checkboxes for filters
        self.chk_function1 = tk.Checkbutton(self.checkbox_frame, text="Filter RGB", variable=self.func1_var, bg="pink")
        self.chk_function1.grid(row=1, column=0, sticky="w")

        self.chk_function2 = tk.Checkbutton(self.checkbox_frame, text="Video Shifting", variable=self.func2_var, bg="pink", command=self.toggle_func2_fields)
        self.chk_function2.grid(row=2, column=0, sticky="w")

        self.chk_function3 = tk.Checkbutton(self.checkbox_frame, text="Filter Canny", variable=self.func3_var, bg="pink")
        self.chk_function3.grid(row=3, column=0, sticky="w")

        self.chk_function4 = tk.Checkbutton(self.checkbox_frame, text="Filter Bilateral", variable=self.func4_var, bg="pink", command=self.toggle_func4_fields)
